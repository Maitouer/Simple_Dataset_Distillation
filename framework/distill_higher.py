...
class Distill(nn.Module):
    ...
    def subsample(self):
        ...
        if self.task_sampler_nc == self.num_classes:
            for i in range(self.num_classes):
                ind = (
                    torch.randperm(self.img_pc)[: self.batch_pc].sort()[0]
                    + self.img_pc * i
                )
                indices.append(ind)
        else:
    ...
    def forward(self, x):
        self.net = get_arch(
            self.arch, self.num_classes, self.channel, self.im_size
        ).cuda()
        self.net.train()
        ...
        if self.dd_type == "curriculum":
            for i in range(self.curriculum):
                ...
                imgs = self.syn_intervention(
                    imgs, dtype="syn", seed=random.randint(0, 10000)
                )
                ratio = 0
