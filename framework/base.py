...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.limit_train:
        train1 = torch.utils.data.Subset(train1, range(5000))
    ...
    if args.ckptname != "none":
        ...
        if args.train_y:
            ...
            model.label.weight.data = label_data
    ...
    if args.load_ckpt:
        ...
        model.data.weight.data = base_data
        if args.train_y:
            ...
            if test_loss < best_loss1:
                ...
                if epoch >= best_loss_ind + 60:
                    ...
                    if args.ddtype == "curriculum":
                        if args.cctype == 0:
                            if model.curriculum == args.minwindow:
                                break
                        elif args.cctype == 1:
    ...
    if model.data.weight.get_device() == 0:
        ...
        print(best_rec)
        if args.wandb:
            wandb.log({"best_acc": best_rec["acc"]})
...
def train(train_loader, model, criterion, optimizer, epoch, device, distill_steps, args):
    ...
    for train1 in enumerate(train_loader):
        ...
        if args.train_y:
            with torch.no_grad():
                model.label.weight.data = torch.clip(model.label.weight.data, min=0)
        ...
        # print the results
        if (i + 1) % args.print_freq == 0 and model.data.weight.get_device() == 0:
    ...
    def run_validate(loader, base_progress=0):
        with torch.no_grad():
            ...
            for i, (images, target) in enumerate(loader):
                ...
                # if (i+1) % args.print_freq == 0 and model.data.get_device() == 0:
                #    progress.display(i + 1)
