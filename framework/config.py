...
from framework.convnet import ConvNet
...
def get_dataset(dataset, root, transform_train, transform_test, zca=False):
    ...
    if dataset == "cifar10":
        if zca:
            ...
            trainset.data, testset.data, process_config = preprocess(
                trainset.data, testset.data, regularization=0.1
            )
        ...
        trainset = ImageFolder(
            os.path.join(data_path, "train"), transform=data_transforms["train"]
        )  # no augmentation
        testset = ImageFolder(
            os.path.join(data_path, "val"), transform=data_transforms["val"]
        )
...
class cub200(torch.utils.data.Dataset):
    def __init__(self, root, train=True, transform=None):
        ...
        if self._check_processed():
            print(
                ...
                if self.train
                else "Test file has been extracted"
            )
        else:
            self._extract()
    ...
    def _check_processed(self):
        ...
        return os.path.isfile(
            os.path.join(self.root, "processed/train.pkl")
        ) and os.path.isfile(os.path.join(self.root, "processed/test.pkl"))
    ...
    def _extract(self):
        ...
        print(
            "Total images: {}, training images: {}. testing images: {}".format(
