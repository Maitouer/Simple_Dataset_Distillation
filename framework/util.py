...
def init_gaussian(num_classes, ipc_per_class, tensor_length):
    ...
    class_means = torch.normal(
        torch.zeros(num_classes, tensor_length), torch.ones(tensor_length)
    )
    ...
    for i in range(num_classes):
        for j in range(ipc_per_class):
            tensors[i * ipc_per_class + j] = torch.normal(
                class_means[i], variance * torch.ones(tensor_length)
            )
...
class ImageIntervention(object):
    def __init__(self, name, strategy, phase, not_single=False):
        ...
        if self.name in ["syn_aug", "real_aug", "pair_aug"]:
            self.functions = {
                ...
                "crop": self.diff_crop,
                "color": [
                    self.diff_brightness,
                    self.diff_saturation,
                    self.diff_contrast,
                ],
    ...
    def do(self, x, seed):
        if not self.not_single:
            ...
            if intervention == "color":
                self.set_seed(seed)
                function = self.functions["color"][
                    np.random.randint(0, len(self.functions["color"]), size=(1,))[0]
                ]
            else:
            ...
            if self.color:
                for f in self.functions["color"]:
                    ...
                    x = f(x)
            if len(self.keys) > 0:
                intervention = self.keys[
                    np.random.randint(0, len(self.keys), size=(1,))[0]
                ]
                function = self.functions[intervention]
                x = function(x)
    ...
    def diff_scale(self, x):
        ...
        sx = torch.Tensor(
            np.random.rand(x.shape[0]) * (ratio - 1.0 / ratio) + 1.0 / ratio
        )
        self.update_seed()
    ...
    def diff_rotate(self, x):
        ...
        theta = (
            torch.Tensor(np.random.rand(x.shape[0]) - 0.5)
            * 2
            * ratio
            / 180
            * float(np.pi)
    ...
    def diff_crop(self, x):
        ...
        translation_x = (
            torch.Tensor(
                np.random.randint(-shift_x, shift_x + 1, size=[x.size(0), 1, 1])
            )
            .to(x.device)
            .long()
        )
        ...
        translation_y = (
            torch.Tensor(
                np.random.randint(-shift_y, shift_y + 1, size=[x.size(0), 1, 1])
            )
            .to(x.device)
            .long()
        )
